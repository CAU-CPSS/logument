...
func Test_jpatch(t *testing.T) {
	...
	var fromDoc interface{}
	if err := json.Unmarshal([]byte(superComplexBase), &fromDoc); err != nil {
		panic(err)
	}

	var toDoc interface{}
	if err := json.Unmarshal([]byte(superComplexA), &toDoc); err != nil {
		panic(err)
	}

	patch, err := jpatch.CreatePatch("", fromDoc, toDoc)
	if err != nil {
		panic(err)
	}
	...
	t.Log("생성된 패치:")
	t.Log(string(patchJSON))

	// 패치를 원본 문서에 적용하여 변경된 문서를 생성합니다.
	if err := jpatch.ApplyPatch(&fromDoc, patch); err != nil {
		panic(err)
	}

	result, err := json.MarshalIndent(fromDoc, "", "  ")
	if err != nil {
		panic(err)
	}
